...
class MakeAppConfig(Distribution):
    ...
    pour construire une application Windows avec des assets,
    ...
    def __init__(self) -> None:
        super().__init__("Windows", is_library=False)
        ...
        self.compiler = VarEnum(
            ["MSVC"], "MSVC", "Compiler used for the application"
        )

    def validate(self) -> bool:
        if self.lib_api.to_data() != "SDL2":
            raise ValueError("Only SDL2 is supported for now.")
        if self.compiler.to_data() != "MSVC":
