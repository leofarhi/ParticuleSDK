...
void* malloc(size_t size) {
    ...
    void* ptr = gint::kmalloc(size, NULL);
    track_ptr(ptr);
    return ptr;
}
...
void* calloc(size_t count, size_t size) {
    void* ptr = gint::kmalloc(count * size, NULL);
    if (ptr) {
        track_ptr(ptr);
...
static void memtrack_report_leaks(void) {
    ...
    if(memtrack_alloc_count > 0) {
        ...
        gint::dprint(1, 1, gint::C_WHITE, buffer);
    } else {
        gint::dprint(1, 1, gint::C_WHITE, "No memory leaks!");
    }
    gint::dupdate();
    gint::getkey(); // pause
}
