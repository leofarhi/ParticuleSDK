...
namespace Particule::Core
{
    ...
    struct Character {
        ...
        int draw_scaled(int x, int y, const Color& color, fixed12_32 ratio) const
        {
            ...
            int dst_x1 = std::min(x + int(scaled_width), window->Width());
            int dst_y1 = std::min(y + int(scaled_height), window->Height());
            ...
            draw_y_start = std::clamp(draw_y_start, 0, window->Height());
            draw_y_end   = std::clamp(draw_y_end,   0, window->Height());
        ...
        int draw(int x, int y, const Color& color) const
        {
            ...
            int dst_x0 = std::max(x, 0);
            int dst_y0 = std::max(y, 0);
            int dst_x1 = std::min(x + width(), window->Width());
            int dst_y1 = std::min(y + height(), window->Height());
    ...
    private:
        ...
        inline Character* get(char32_t codepoint)
        {
